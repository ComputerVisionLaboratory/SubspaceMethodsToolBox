classdef cvlKCMSM
% Class for applying kernel constraint MSM (KCMSM)
% Gaussian kernel is used
%
% List of function:
%   cvlKCMSM: initialization: generate orthogonal generalized subspace
...
% http://www.cvlab.cs.tsukuba.ac.jp/
    ...
    methods
        function OB = cvlKCMSM(X1, nSubDim1,nSigma)
        % Initialization: generate the orthogonal basis of generalized
        % subspace
        % Parameters:
        ...
        %  nSigma: parameter for Gaussian kernel bandwidth
            ...
            [B, OB.W] = eig(OB.kMat);
            [OB.W, ind] = sort(diag(OB.W),'ascend');
            B = B(:,ind);
            OB.O = diag(1./sqrt(OB.W))*B';                        
        end
        
        function [basisVect, eigVal2, eigRat2, eigVec2] = TransformS(OB, X2,nSubDim2,nSubDim3)
        % Function to generate subspaces which are projected to the 
        % generalized constraint subspace: apply KPCA, then project to
        % generalized constraint subspace, finally apply orthogonalization 
        % using Gram Schmidt 
        % Parameters:
        ...
        %   nSubDim3: the dimension of generalized constraint subspace,
        %       i.e., the number of smallest principal components to
        %       keep for the generalized constraint subspace
        % Return values:
        %   basisVect: set of basis vectors obtained. The output
        %       will always be in the form of cell if there are more than one 
        %       set of data in X2
        %   eigVal2, eigRat2, eigVec2: eigenvalues, cumulative ratio,
        %       and eigenvectors obtained from kernel PCA in the form of cell
        %       if the are more than one set of data in X2
            if (iscell(X2))
                nSet2 = numel(X2);
                ...
                for i = 1:nSet2
                    ...
                    basisVect{i} = cvlGramSchmidt(OB.O(1:nSubDim3,:)*aFinal*eigVec2{i});
                end
                if (nSet2==1)
        ...
        % Function to project the data X2 to the orthogonal generalized subspace
        % Parameters:
        %   X2: input data in the form of cell or multidimensional
        %       matrix (similar format as initialization)
        % Return values:
        %  transformedData: always in the form of cell if there are more than
